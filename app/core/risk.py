...
신호를 뽑아 넘기면 동일한 기준으로 LOW/MEDIUM/HIGH/CRITICAL을 돌려준다.

(scenario 위험 노드가 이걸 호출. testcase의 risk 노드 등에서도 동일 기준 재사용.)

NOTE: 여기서 산정하는 건 '위험도 심각도'(RiskLevel: LOW/MEDIUM/HIGH/CRITICAL)이다.
      테스트 범위/레벨(TestLevel: SMOKE/SANITY/REGRESSION/FULL_SUITE)과는 다른 축이며,
      섞지 말 것. test_level은 planner가 별도로 산정한다.

기준 (우선순위 순):
- CRITICAL: 파괴적(DELETE)이면서 인증/음성 테스트를 동반,
            또는 인증 보호 + 변경(write) 자원에 대해 인증 우회를 탐침(AUTHORIZATION)
- HIGH:     파괴적이거나 / 변경 + 인증 보호 동시 / 음성 테스트 2건 이상 /
            깊은 체이닝(>=3) 또는 긴 시나리오(>=6 step)
- MEDIUM:   변경이 있거나 / 인증 보호가 있거나 / 음성 1건 / 체이닝 존재
- LOW:      그 외 (대개 읽기 전용·비인증·단순 흐름)
