...
3. 식별자 연결: 이전 응답의 ID(예: userId, postId)가 다음 스텝의 path 파라미터나 body의 외래 키로 자연스럽게 연결되면 매핑하세요.
   - [플레이스홀더 해소] 어떤 스텝의 requestSpec.path에 `{appId}` 같은 **미해결 플레이스홀더**가 남아 있으면,
     그 값을 채울 수 있는 앞선 스텝(목록/조회 응답)을 찾아 그 응답의 실제 id를 해당 path 파라미터로 주입하는
     매핑을 반드시 만드세요. (planner가 유효 ID 확보를 위해 조회 스텝을 앞에 넣고 플레이스홀더를 남겨둔 것입니다.)
     이때 target_location="path", target_field=파라미터명(예: "appId")으로 둡니다.
   - [중요/덮어쓰기 금지] 음성 케이스(잘못된 입력·없는 id로 4xx 유도)에서 requestSpec에 **이미 들어 있는
     invalid/빈 값**(예: pathParams의 ""나 존재하지 않는 id)은 의도된 실패값입니다. 절대 매핑으로 덮어쓰지 마세요.
     플레이스홀더(`{...}`)가 남아 있는 경우에만 채웁니다.
4. JSONPath 정확성: source_json_path는 반드시 source 스텝의 response_schema에 실제 존재하는 경로여야 합니다. 임의의 경로를 만들면 안 됩니다.
   - 응답이 리스트(배열)면 요소 하나를 선택해야 합니다. 보통 첫 요소를 쓰며 `$[0].id`, `$.data[0].appId`처럼
     인덱스를 포함한 경로로 작성하세요.
...
- 어떤 스텝의 requestSpec.path에 `{{...}}` 플레이스홀더가 남아 있으면, 앞선 조회/목록 스텝의 응답에서
  그 값을 뽑아 채우는 매핑을 만드세요. 단, 음성 케이스에 의도적으로 넣은 invalid/빈 값은 그대로 둡니다.
