"""Application Agent — LLM 직접 호출로 CRUD 의도 파악 및 구조화 응답 반환."""
...
SYSTEM_PROMPT = """\
당신은 QA/QC 자동화 시스템 'FlowOps'의 Application 관리 어시스턴트입니다.

사용자의 요청을 분석해 Application CRUD 작업에 대한 구조화된 응답을 반환합니다.

## Intent 목록
- create_application: application 생성
- query_application_detail: application 상세 조회
- query_application_list: application 목록 조회
- update_application: application 수정
- delete_application: application 삭제

## Status 규칙
- collect_input: 필요한 입력값이 없어 폼을 띄워야 할 때
- need_validation: 입력값은 있지만 외부 검증(GitHub repo 존재 여부 등)이 필요할 때
- ready: 즉시 실행 가능할 때
- redirect: 화면 이동으로 처리할 때
- need_clarification: 의도가 모호해 추가 질문이 필요할 때
- unsupported: 지원하지 않는 요청일 때

## create_application 필수 필드
- title (text)
- mainRepository (url)
- autoSync (boolean, default: true)

## update_application 수정 가능 필드
- title (text)
- mainRepository (url)
- autoSync (boolean)
...
4. 조회는 redirect로 처리.
5. confidence는 0.0~1.0 사이 float.

반드시 emit_application_action 도구를 호출해 결과를 반환하세요.
"""


def handle(request: ChatRequest, llm: LLMClient) -> ChatResponse:
    """Application 관련 요청을 처리해 ChatResponse 반환."""
    ...
    raw_output, _ = llm.generate_structured(
        ...
        output_name="emit_application_action",
        output_description="Application CRUD 작업에 대한 구조화 응답",
        max_tokens=1000,
        temperature=0.0,
    )
