...
스키마 변경 반영:
- step에 chained_variables만 갱신하므로 model_copy(update=...)로 처리.
  ScenarioStep/Scenario 필드가 늘어나도(type/test_level 등) update에 없는 필드는
  자동 보존되므로 이 코드는 영향받지 않는다.
"""
...
def _chain_one_scenario(
    ...
    # 원본 step에 chained_variables만 갱신해 복사 (다른 필드는 그대로 보존).
    new_steps = [
