...
def test_read_only_simple_is_low():
    """읽기 전용·비인증·단순 → LOW."""
    assert assess_risk(_sig()) == RiskLevel.LOW


def test_mutating_only_is_medium():
    """변경만 있으면 → MEDIUM."""
    assert assess_risk(_sig(mutating=True)) == RiskLevel.MEDIUM


def test_auth_protected_read_is_medium():
    """인증 보호된 읽기 → MEDIUM."""
    assert assess_risk(_sig(auth_protected=True)) == RiskLevel.MEDIUM


def test_single_negative_is_medium():
    """음성 테스트 1건 → MEDIUM."""
    assert assess_risk(_sig(negative_count=1)) == RiskLevel.MEDIUM


def test_chaining_only_is_medium():
    """체이닝만 있어도 → MEDIUM."""
    assert assess_risk(_sig(chain_depth=1)) == RiskLevel.MEDIUM


def test_mutating_and_auth_is_high():
    """변경 + 인증 보호 동시 → HIGH."""
    assert assess_risk(_sig(mutating=True, auth_protected=True)) == RiskLevel.HIGH


def test_two_negatives_is_high():
    """음성 2건 이상 → HIGH."""
    assert assess_risk(_sig(negative_count=2)) == RiskLevel.HIGH


def test_long_scenario_is_high():
    """긴 시나리오(>=6 step) → HIGH."""
    assert assess_risk(_sig(mutating=True, step_count=6)) == RiskLevel.HIGH


def test_deep_chaining_is_high():
    """깊은 체이닝(>=3) → HIGH."""
    assert assess_risk(_sig(chain_depth=3)) == RiskLevel.HIGH


def test_destructive_is_high_at_least():
    """파괴적(DELETE)이면 최소 HIGH (인증/음성 없을 때)."""
    assert assess_risk(_sig(mutating=True, destructive=True)) == RiskLevel.HIGH
